...
async function runCmd(bin, args = []) {
  ...
  try {
    return await exec(_bin, args, {shell: system.isWindows()});
  } catch (err) {
 ...
 * @param {*} deviceName
 */
export async function listRunningApps(deviceName) {
  //
  //
  //com.suitest.watchme.app
  //another-app
  //
  await runDeviceCmd(ARES_LAUNCH, deviceName, ['--running']);
}
 ...
 * @param {*} deviceName
 */
export async function listApps(deviceName) {
  //
  //
  //com.suitest.watchme.app
  //another-app
  //
  await runDeviceCmd(ARES_INSTALL, deviceName, ['--list']);
}
