...
export async function installApp(ipkPath, appId, deviceName) {
  ...
  const isRemoteApp = ipkPath.startsWith('https://');
  if (isRemoteApp) {
    ipkPath = await downloadRemoteWebosAppToTempFile(ipkPath);
  }
...
function downloadRemoteWebosAppToTempFile(appUrl) {
  const tempDir = mkdtempSync(`${tmpdir()}${path.sep}`);
  const filePath = path.join(tempDir, 'app.ipk');
